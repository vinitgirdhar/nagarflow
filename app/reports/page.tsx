...
function getReportIcon(type: string) {
  return type === 'daily' ? '📋' : type === 'weekly' ? '📊' : type === 'emergency' ? '🚨' : '📈';
}
    ...
    <DashboardShell title="Reports" badges={[{ type: 'live', text: 'Claude API' }]}>
      ...
      {REPORTS.map((r, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '1rem', background: 'var(--dark-surface)', border: '1px solid var(--border-subtle)', borderRadius: '12px', padding: '1.25rem', marginBottom: '.75rem', transition: 'border-color .2s', cursor: 'default' }}
          ...
          <div style={{ fontSize: '28px', width: '48px', height: '48px', borderRadius: '10px', background: 'rgba(193,68,14,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{getReportIcon(r.type)}</div>
          <div style={{ flex: 1 }}>
      ...
      <div style={{ background: 'rgba(122,140,94,.06)', border: '1px solid rgba(122,140,94,.2)', borderRadius: '12px', padding: '1.5rem', marginTop: '2rem' }}>
        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: '18px', color: 'var(--text-heading)', marginBottom: '.75rem', display: 'flex', alignItems: 'center', gap: '.5rem' }}>🤖 AI-Generated Daily Summary</div>
        <div style={{ fontSize: '13px', color: 'var(--secondary)', lineHeight: 1.7 }}>
          ...
          <p style={{ color: 'var(--accent)', marginTop: '.75rem' }}>— Generated by Claude API · UrbanPulse AI Report Engine v2.1</p>
        </div>
      </div>
    </DashboardShell>
  );
